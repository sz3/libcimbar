...
{
	cv::Mat tile = cimbar::load<cv::Mat>::getTile(4, 0, false);
	assertEquals(0x103070f1f3f7f, image_hash::average_hash(tile));

	tile = cimbar::load<cv::Mat>::getTile(4, 0, false);
	assertEquals(0x103070f1f3f7f, image_hash::average_hash(tile));
}

TEST_CASE( "averageHashTest/testDark", "[unit]" )
{
	cv::Mat tile = cimbar::load<cv::Mat>::getTile(4, 0, true);
	assertEquals(0xfffefcf8f0e0c080, image_hash::average_hash(tile));

	tile = cimbar::load<cv::Mat>::getTile(4, 0, true);
	assertEquals(0xfffefcf8f0e0c080, image_hash::average_hash(tile));
}
