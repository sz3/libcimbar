...
{
public:
	SimpleDecoder(int ecc_bytes=-1, int color_bits=-1, bool interleave=true);
...
inline SimpleDecoder::SimpleDecoder(int ecc_bytes, int color_bits, bool interleave)
 ...
 * */
template <typename STREAM>
inline unsigned SimpleDecoder::do_decode(CimbReader& reader, STREAM& ostream, bool legacy_mode)
{
	if (legacy_mode)
...
template <typename STREAM>
inline unsigned SimpleDecoder::do_decode_coupled(CimbReader& reader, STREAM& ostream)
{
...
template <typename MAT, typename STREAM>
inline unsigned SimpleDecoder::decode(const MAT& img, STREAM& ostream, unsigned color_mode, bool should_preprocess, int color_correction)
{
...
inline unsigned SimpleDecoder::decode_fountain(const MAT& img, FOUNTAINSTREAM& ostream,  unsigned color_mode, bool should_preprocess, int color_correction)
{
	CimbReader reader(img, _decoder, color_mode, should_preprocess, color_correction);
