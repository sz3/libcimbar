...
{
public:
	static cv::Vec3b getColor(unsigned index)
	{
	...
	static std::string getTileDir(unsigned symbol_bits)
	{
		return fmt::format("{}/bitmap/{}", LIBCIMBAR_PROJECT_ROOT, symbol_bits);
	}

	static cv::Mat getTile(std::string tile_dir, unsigned symbol, bool dark=true, unsigned color=0)
	{
